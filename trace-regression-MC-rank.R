...
#---------------------------------------------------------------------
# Function to produce response y given (calX, noise, parameter)
#---------------------------------------------------------------------
...
#------------------------------------------------------------------------------------------------------------------
# Function to call data, do trace regression with matrix decomposition for each sparsity on vector vs 
# over grid of ranks for the low-rank parameter. 
# Compute and plot MSE versus Rank averaged over N.rep repetitions for each sparsity in vs. 
# Given repetition, the same data is used across all corruptions. 
# For each rank r, the same low-rank parameter is used across repetitions. 
#------------------------------------------------------------------------------------------------------------------
trace.reg.MD.MSE <- function(n,d1,d2,vs,R,random,sd,B.mag,Gamma.mag,L,t.final,N.rep){
  
  p = d1*d2
  S = dim(as.array(vs))
  
  # Save sparse-parameter
  elm = matrix(rep(0,p), nrow = d1)
  Gamma.true = rep(list(elm), S)
  for (j in 1:S) {
    Gamma.true[[j]] = data_genGamma(d1,d2,vs[j],Gamma.mag,random)
  }

  # Save grid of low-rank parameters
  elm = matrix(rep(0,p), nrow = d1)
  B.true = rep(list(elm), R)
  for (r in 1:R) {
    B.true[[r]] = data_genB(n,d1,d2,r,B.mag)
  }
  
  # Initialization of repetition count
  elm = rep(0,R)
  MSE = rep(list(elm), S)
  elm = matrix( rep(0, t.final*R ) , nrow = t.final )
  Mvalue = rep(list(elm), S)
  ...
  for (index in 1:N.rep) {
    ...
    for (j in 1:S){
      for (r in 1:R) {
        ...
        y = data_genY(n,calX,noise,B.true[[r]],Gamma.true[[j]])
      
        #Regression
        ls1 = trace.reg.MD(n,d1,d2,B.mag,sd,calX,y,L,t.final)
        MSE[[j]][r] = MSE[[j]][r] + norm.sq( ls1$B - B.true[[r]] ) + norm.sq( ls1$Gamma - Gamma.true[[j]] )
        Mvalue[[j]][,r] = Mvalue[[j]][,r] + as.matrix(ls1$value)
      }
    }  
...
#-----------------------------------------------------------------------------------------
#vs = c(10)
#ls1 <- trace.reg.MD.MSE(n,d1,d2,vs[1],R,random,sd,B.mag,Gamma.mag,L,t.final,N.rep)

#Aggregating data points in single data frame factored by the 'sparsity'
#MSEcomb = c( ls1$MSE[[1]] )
#Rcomb = c( 1:R )
#sparsity = as.factor( c( rep(vs[1], R ) ) )
#df <- data.frame( MSE = MSEcomb, Rank = Rcomb, Sparsity = sparsity)
...
vs = c(5,80)
ls1 <- trace.reg.MD.MSE(n,d1,d2,vs[1],R,random,sd,B.mag,Gamma.mag,L,t.final,N.rep)
ls2 <- trace.reg.MD.MSE(n,d1,d2,vs[2],R,random,sd,B.mag,Gamma.mag,L,t.final,N.rep)

#Aggregating data points in single data frame factored by the 'sparsity'
MSEcomb = c( ls1$MSE[[1]], ls2$MSE[[1]] )
Rcomb = c( 1:R, 1:R )
sparsity = as.factor( c( rep(vs[1], R ), rep( vs[2], R ) ) )
df <- data.frame( MSE = MSEcomb, Rank = Rcomb, Sparsity = sparsity)
...
pl <- ggplot( data = df, aes(x = Rank, y = MSE, color = Sparsity, shape = Sparsity ))
pl <- pl + geom_point(size = 4, alpha = 1) +  ylim(0, 3)
pl <- pl + geom_smooth(method = 'lm',se=FALSE, linetype="dashed")
pl <- pl + theme_bw()
pl <- pl + theme(text=element_text(family="serif",size=30)) + labs( x =  'Rank', y = 'MSE')
